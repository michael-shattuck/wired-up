...
export type RegisteredServiceName =
  'logger'
  | 'config'
  | 'db'
  | 'deeperFunction';
...
export class Container {
  ...
  private _singletons = new Map<RegisteredServiceName, any>();
  private _services = new Map<RegisteredServiceName, RegisteredService<any>>();
  ...
  public static async destroy() {
    ...
    const singletons = Array
      .from(Container._instance._services.entries())
      .filter(([_, service]) => service.type === 'singleton');
   ...
   */
  public static async startScope(next: (...args: any[]) => any) {
    return RequestScope.run(async () => {
      const scopedServices = Array
        .from(Container._instance._services.entries())
        .filter(([_, service]) => service.type === 'scoped');
   ...
   * @returns {Promise<(...args: any[]) => any>} The receiving function with services injected
   */
  public async inject(serviceNames: RegisteredServiceName[], receivingFn: (...args: any[]) => (...args: any[]) => any): Promise<(...args: any[]) => any> {
   ...
   * @returns {Promise<(...args: any[]) => any>} The resolved function
   * @throws {Error} If the function has parameters that are not registered services
   */
  public async resolve(func: (...args: any[]) => any): Promise<(...args: any[]) => any> {
    if (func.length === 0) {
      return await func();
    }

    const params = describeParams(func) as RegisteredServiceName[];
    ...
    return await this.inject(params, func);
  }

  private async setupSingletons(): Promise<void> {
    const singletons = Array
      .from(this._services.entries())
      .filter(([_, service]) => service.type === 'singleton');
  ...
  private async getService(serviceName: RegisteredServiceName): Promise<any> {
    ...
    if (service.type === 'singleton') {
      const singleton = this._singletons.get(serviceName);
      if (!singleton) {
...
export function singleton<TService>(name: RegisteredServiceName, setupFn: (...args: any[]) => Promise<TService>, teardownFn?: (...args: any[]) => Promise<void>): RegisteredService<TService> {
  return {
    name,
    type: 'singleton',
    setupFn,
...
export function scoped<TService>(name: RegisteredServiceName, setupFn: (...args: any[]) => Promise<TService>, teardownFn?: (...args: any[]) => Promise<void>): RegisteredService<TService> {
  return {
    name,
    type: 'scoped',
    setupFn,
...
export function transient<TService>(name: RegisteredServiceName, setupFn: (...args: any[]) => Promise<TService>, teardownFn?: (...args: any[]) => Promise<void>): RegisteredService<TService> {
  return {
    name,
    type: 'transient',
    setupFn,
